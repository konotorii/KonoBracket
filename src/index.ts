  ...
  for (let team in teams) {
    if (teams[team].p1_id !== null) {
      ...
      while (count <= env.player_per_team) {
        let res
        try {
          res = await superagent.get(`https://osu.ppy.sh/api/get_user?k=${env.osu_api_key}&u=${teams[team][`p${count}_id`]}&type=id`)
        }
        catch (err) {
          res = null
        }

        const player = res !== null ? res.body : undefined
        // consola.log(player)
        if (player == undefined) {
...
async function scoring(method: Scoring,) {
  if (method == "avg_sum_of_placements") {

  }
  else if (method == "max_percent") {

  }
  else if (method == 'percent_difference') {

  }
  else if (method == 'z_sum') {
...
async function getMapInfo(id: string) {
  try {
    const res = await superagent.get(`https://osu.ppy.sh/api/get_beatmaps?k=${env.osu_api_key}&b=${id}`)
    const data = res.body
    return data
  }
  catch (err) {
